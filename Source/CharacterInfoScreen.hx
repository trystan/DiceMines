...
class CharacterInfoScreen extends Screen {
    ...
    private function draw(display:AsciiDisplay):Void {
        ...
        y++;
        display.write("Stats: (with equp)  [ min agv max ]", x-1, y++, light);
        display.write("accuracy:   " + breakdown(player.effectiveAccuracyStat("idle")), x, y++);
        display.write("damage:     " + breakdown(player.effectiveDamageStat("idle")), x, y++);
        display.write("evasion:    " + breakdown(player.effectiveEvasionStat("idle")), x, y++);
        display.write("resistance: " + breakdown(player.effectiveResistanceStat("idle")), x, y++);
        display.write("piety:      " + breakdown(player.effectivePietyStat("idle")), x, y++);
        y++;
